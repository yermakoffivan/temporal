...
func mustNewCallbackValidator() callbacks.Validator {
	...
	cfg := callbacks.ValidatorConfig{
		...
		WorkerSourceContextAggregateMaxSize: func(string) int { return 1500 },
	}
...
// newWorkerCallback returns a Worker-variant callback whose source context serializes to approximately sourceCtxSize bytes.
func newWorkerCallback(sourceCtxSize int) *commonpb.Callback {
...
func enableWorkerCallbacks(c *Config) {
	c.EnabledCallbackKinds = func(string) []callbacks.Kind {
		return []callbacks.Kind{callbacks.KindNexus, callbacks.KindWorker}
	}
}
...
func TestValidateStartNexusOperationExecutionRequest(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			mutate: func(r *workflowservice.StartNexusOperationExecutionRequest) {
				r.CompletionCallbacks = []*commonpb.Callback{newWorkerCallback(0)}
			},
			wantErr: "worker callbacks are not enabled for this execution type",
		},
		{
			name: "source_context - rejects a single callback over the per-callback limit",
			mutate: func(r *workflowservice.StartNexusOperationExecutionRequest) {
				r.CompletionCallbacks = []*commonpb.Callback{newWorkerCallback(1001)}
			},
			mutateConfig: enableWorkerCallbacks,
...
// Tests for the the aggregate cap on worker-callback source context payloads.
func TestValidateStartRequestSourceContextAggregate(t *testing.T) {
	...
	enableWorkerCallbacks(config)
	v := newTestValidator(config)

	t.Run("AcceptsCallbacksWithinTheAggregateLimit", func(t *testing.T) {
		err := v.validateAndNormalizeStartRequest(ctx, newRequest(newWorkerCallback(900)))
		require.NoError(t, err)
	})
	...
	t.Run("RejectsCallbacksOverTheAggregateLimitTogether", func(t *testing.T) {
		err := v.validateAndNormalizeStartRequest(
			ctx,
			newRequest(newWorkerCallback(900), newWorkerCallback(900)))
	...
	t.Run("NexusCallbacksCarryNoSourceContext", func(t *testing.T) {
		cbs := []*commonpb.Callback{newWorkerCallback(900)}
		for range 5 {
